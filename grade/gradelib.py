...
def assert_lines_match(text, *regexps, **kw):
    ...
    *not* match any line in text."""
    ...
    for i, line in enumerate(lines):
        if re.search(regexps[0], line):
            # Check if the rest of the regexps match the following lines
            all_match = True
            for j in range(1, regexps_len):
                if i+j >= lines_len or not re.search(regexps[j], lines[i+j]):
                    all_match = False
                    i = i+j
                    break
...
""")
    ...
    def match(self, *args, **kwargs):
        ...
        # remove pa
        self.qemu.output = re.sub(r'pa=[^ ]* ', '', self.qemu.output)
